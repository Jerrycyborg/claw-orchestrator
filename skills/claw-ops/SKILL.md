---
name: claw-ops
description: Assess operational readiness before a change goes out — rollout sequencing, pre-flight checks, what to watch, and a rollback path written before anything is touched, ending in an explicit go/no-go with mitigations. Use before deploying, releasing, migrating, or any action against production. Applies claw data-handling rules (no secrets, bounded evidence).
---

# Ops

The Ops role from `config/orchestrator.yaml`. Runtime-free.
